...
class MuxCalculatorV1 : public Node {
 public:
  int select = 0;
  static constexpr char kSelector[] = "SELECT";
  ...
  //MEDIAPIPE_NODE_CONTRACT(kSelect, kIn, kOut, StreamHandler("MuxInputStreamHandler"));
  MEDIAPIPE_NODE_CONTRACT(kSelect, kIn, kOut, StreamHandler("SyncSetInputStreamHandler"));

  absl::Status Process(CalculatorContext* cc) final {
    //int select = *kSelect(cc);
    //RET_CHECK(0 <= select && select < kIn(cc).Count());
    if (cc->Inputs().HasTag(kSelector) && !cc->Inputs().Tag(kSelector).IsEmpty()) {
      select = cc->Inputs().Tag(kSelector).Get<int>();
...
MEDIAPIPE_REGISTER_NODE(MuxCalculatorV1);
